...
import { ArrowUpRight, Check, ChevronRight, CircleHelp, Clock3, EyeOff, ExternalLink, FileSearch, Film, History, RadioTower, Search, ShieldCheck, Sparkles, X } from 'lucide-vue-next'
...
const unresolvedCandidates = computed(() => allCandidates.value.filter((item: any) => normalizedCandidateStatus(item) === 'candidate'))
...
const statusOptions = [
  { value: 'candidate', label: 'Candidates requiring action' },
  { value: 'idea_created', label: 'Idea created' },
  { value: 'hidden', label: 'Hidden' },
  { value: 'all', label: 'All statuses' },
]
function normalizedCandidateStatus(item: any) {
  if (item.idea_id || ['selected', 'idea_created'].includes(item.status)) return 'idea_created'
  if (['hidden', 'muted', 'rejected'].includes(item.status)) return 'hidden'
  return 'candidate'
}
const candidates = computed(() => allCandidates.value.filter((item: any) => {
  const query = candidateSearch.value.trim().toLowerCase()
  const matchesSearch = !query || [item.title, item.angle, item.audience, item.candidate_type, item.target_audience_insight, item.content_goal, item.core_message, item.problem_or_tension, item.proposed_solution, item.informational_value, item.entertainment_hook, item.virality_mechanism, item.creative_direction, ...(item.suggested_formats || []), ...(item.suitable_visual_modes || [])].some(value => String(value || '').toLowerCase().includes(query))
  ...
  const matchesStatus = statusFilter.value === 'all' || normalizedCandidateStatus(item) === statusFilter.value
...
const selectedEvidenceLinks = computed(() => {
  ...
  const links = [
    ...(selectedEvidence.value.url ? [{ url: selectedEvidence.value.url, label: selectedEvidence.value.title || 'Open source' }] : []),
    ...((selectedEvidence.value.links || []).map((item: any) => typeof item === 'string' ? { url: item, label: item } : item)),
  ]
  return links.filter((item: any, index: number, items: any[]) => item?.url && items.findIndex(candidate => candidate.url === item.url) === index)
})
  ...
  try {
    for (let index = 0; index < 240; index += 1) {
      ...
      if (['completed', 'failed'].includes(state.status)) {
        await refresh()
        await refreshNuxtData('shell-research-candidates')
...
async function selectCandidate(candidate: any) {
  ...
  await refresh()
  await refreshNuxtData('shell-research-candidates')
  selectedCandidate.value = allCandidates.value.find((item: any) => item.id === candidate.id) || result
}

function openCandidate(candidate: any) {
  if (candidate.idea_id) return navigateTo(`/ideas?idea=${candidate.idea_id}`)
  selectedCandidate.value = candidate
}

function openEvidence(source: any) {
  selectedEvidence.value = source
}

function openIdea(candidate: any) {
  if (candidate.idea_id) return navigateTo(`/ideas?idea=${candidate.idea_id}`)
}

function selectResearchRun(run: any) {
...
<template>
  <div>
    ...
    <div class="research-summary">
      ...
      <UiAppCard><span>Unresolved candidates</span><strong>{{ unresolvedCandidates.length }}</strong><small>{{ latestRun?.candidate_count != null ? `${latestRun.candidate_count} from latest run` : 'Awaiting an editorial decision' }}</small></UiAppCard>
    ...
    <div class="grid-two research-layout">
      <UiAppCard>
        ...
        <div v-else class="candidate-list">
          <article v-for="candidate in candidates" :key="candidate.id" class="candidate-card" :class="`candidate-card--${normalizedCandidateStatus(candidate)}`" tabindex="0" role="button" @click="openCandidate(candidate)" @keydown.enter="openCandidate(candidate)">
            <div class="candidate-card__score"><strong>{{ candidate.topic_opportunity_score }}</strong><span>topic</span></div>
            <div class="candidate-card__body"><div class="candidate-card__meta"><UiStatusBadge :status="normalizedCandidateStatus(candidate)" /><span class="candidate-type">{{ String(candidate.candidate_type || 'problem_solution').replaceAll('_', ' ') }}</span><span><Clock3 :size="12" /> {{ freshnessLabel(candidate.freshness_expires_at) }}</span><span><ShieldCheck :size="12" /> {{ Math.round((candidate.score_confidence || 0) * 100) }}% confidence</span></div><h3>{{ candidate.title }}</h3><UiMarkdownText class="candidate-summary" :text="candidate.angle" /><div class="candidate-recommendation"><span><Film :size="12" /> {{ String(candidate.recommended_visual_mode || 'ugc_creator').replaceAll('_', ' ') }}</span><span>{{ candidate.recommended_duration_seconds || 30 }} sec</span><span>{{ candidate.recommended_scene_count_min || 4 }}–{{ candidate.recommended_scene_count_max || 6 }} scenes</span></div><div class="candidate-card__footer"><span>{{ candidate.source_ids?.length || 0 }} cited sources</span><div class="candidate-actions"><button v-if="normalizedCandidateStatus(candidate) === 'candidate'" class="text-link" @click.stop="hideCandidate(candidate)"><EyeOff :size="13" /> Hide</button><button class="text-link" @click.stop="selectedCandidate = candidate"><FileSearch :size="13" /> Details</button><button v-if="candidate.idea_id" class="text-link" @click.stop="openIdea(candidate)">Open idea <ChevronRight :size="13" /></button><button v-else-if="normalizedCandidateStatus(candidate) === 'candidate'" class="text-link" @click.stop="selectCandidate(candidate)">Turn into idea <ChevronRight :size="13" /></button></div></div></div>
          </article>
        </div>
      </UiAppCard>
      ...
      <div class="stack">
        <UiAppCard>
          ...
          <div v-if="latestRun?.sources?.length" class="evidence-list">
            <button v-for="source in latestRun.sources" :key="source.id" type="button" class="evidence-row" :aria-label="`Read full evidence: ${source.title}`" @click="openEvidence(source)"><span class="evidence-row__index">{{ source.id?.split('_').at(-1) }}</span><div><strong>{{ source.title }}</strong><UiMarkdownText :text="source.excerpt" /><small>{{ source.source_type }} · relevance {{ Math.round((source.relevance || 0) * 100) }}%</small></div><FileSearch :size="14" /></button>
          </div>
    ...
    <UiAppCard class="research-history" data-testid="research-history">
      <div class="section-heading"><div><h2>Research history</h2><p>Inspect what was searched, which preference signals were applied, and what each run produced.</p></div><History :size="18" /></div>
      <div v-if="data.runs?.length" class="research-history__list">
        <button v-for="run in data.runs" :key="run.id" type="button" :class="{ active: latestRun?.id === run.id }" @click="selectResearchRun(run)">
          <div><strong>{{ run.objective }}</strong><span>{{ run.trigger_type || 'manual' }} · {{ runDate(run.started_at || run.created_at) }} · {{ runDuration(run) }}</span></div>
          <div class="research-history__metrics"><span>{{ run.sources?.length || 0 }} sources</span><span>{{ run.candidate_count ?? run.candidate_ids?.length ?? 0 }} / {{ run.max_candidates || 5 }} candidates</span><span>{{ run.preference_context?.selected_count || 0 }} selected signals · {{ run.preference_context?.hidden_count || 0 }} hidden signals</span><UiStatusBadge :status="run.status" /></div>
          <small v-if="run.search_queries?.length">Queries: {{ run.search_queries.join(' · ') }}</small>
        </button>
      </div>
      <div v-else class="mini-empty"><History :size="20" /><p>Completed and active research runs will remain available here.</p></div>
    </UiAppCard>
    ...
    <div v-if="selectedCandidate" class="modal-backdrop" data-testid="candidate-details" @click.self="selectedCandidate = null">
      <section class="modal candidate-modal">
        <div class="modal__header"><div><span class="eyebrow">Research candidate · {{ String(selectedCandidate.candidate_type || 'problem_solution').replaceAll('_', ' ') }}</span><h2>{{ selectedCandidate.title }}</h2><UiMarkdownText :text="selectedCandidate.why_now" /></div><button class="icon-button icon-button--plain" aria-label="Close candidate details" @click="selectedCandidate = null"><X :size="18" /></button></div>
        <div class="modal__body candidate-detail">
          <div class="candidate-detail__main">
            <h3>Editorial angle</h3><UiMarkdownText :text="selectedCandidate.angle" />
            <h3>Audience insight</h3><UiMarkdownText :text="selectedCandidate.target_audience_insight || selectedCandidate.audience" />
            <h3>Goal and core message</h3><UiMarkdownText :text="[selectedCandidate.content_goal, selectedCandidate.core_message].filter(Boolean).join('\n\n')" />
            <h3>Problem and proposed solution</h3><UiMarkdownText :text="[selectedCandidate.problem_or_tension, selectedCandidate.proposed_solution].filter(Boolean).join('\n\n')" />
            <h3>Value and viral mechanism</h3><UiMarkdownText :text="[selectedCandidate.informational_value, selectedCandidate.entertainment_hook, selectedCandidate.virality_mechanism].filter(Boolean).join('\n\n')" />
            <h3>Creative direction</h3><UiMarkdownText :text="selectedCandidate.creative_direction" />
            <h3>Recommended production</h3><UiMarkdownText :text="selectedCandidate.format_rationale || 'The recommendation balances the message, audience and available evidence.'" />
            <div class="candidate-recommendation candidate-recommendation--detail"><span><Film :size="13" /> {{ String(selectedCandidate.recommended_visual_mode || 'ugc_creator').replaceAll('_', ' ') }}</span><span v-for="mode in selectedCandidate.suitable_visual_modes || []" :key="mode">also {{ String(mode).replaceAll('_', ' ') }}</span><span>{{ selectedCandidate.recommended_duration_seconds || 30 }} seconds</span><span>{{ selectedCandidate.recommended_scene_count_min || 4 }}–{{ selectedCandidate.recommended_scene_count_max || 6 }} scenes</span></div>
            <h3>Supported claims</h3><ul v-if="selectedCandidate.supported_claims?.length"><li v-for="claim in selectedCandidate.supported_claims" :key="claim.id || claim.claim"><UiMarkdownText :text="claim.claim || claim.text" /></li></ul><p v-else class="muted-copy">No factual claim is promoted without attached evidence.</p>
            <h3>Unresolved questions</h3><ul v-if="selectedCandidate.unresolved_questions?.length"><li v-for="question in selectedCandidate.unresolved_questions" :key="question"><UiMarkdownText :text="question" /></li></ul><p v-else class="muted-copy">No unresolved questions were reported.</p>
          </div>
          <aside><dl><div><dt>Status</dt><dd>{{ normalizedCandidateStatus(selectedCandidate).replaceAll('_', ' ') }}</dd></div><div><dt>Candidate type</dt><dd>{{ String(selectedCandidate.candidate_type || 'problem_solution').replaceAll('_', ' ') }}</dd></div><div><dt>Audience</dt><dd>{{ selectedCandidate.audience }}</dd></div><div><dt>Objective</dt><dd>{{ selectedCandidate.objective }}</dd></div><div><dt>Editorial format</dt><dd>{{ selectedCandidate.format }}</dd></div><div><dt>Opportunity</dt><dd>{{ selectedCandidate.topic_opportunity_score }}</dd></div><div><dt>Confidence</dt><dd>{{ Math.round((selectedCandidate.score_confidence || 0) * 100) }}%</dd></div></dl><h3>Sources</h3><a v-for="source in selectedSources" :key="source.id" :href="source.url" target="_blank" rel="noreferrer">{{ source.title }} <ExternalLink :size="12" /></a></aside>
        </div>
        <div class="modal__footer"><button v-if="normalizedCandidateStatus(selectedCandidate) === 'candidate'" class="button" @click="hideCandidate(selectedCandidate)"><EyeOff :size="14" /> Hide</button><button v-if="selectedCandidate.idea_id" class="button button--primary" @click="openIdea(selectedCandidate)">Open idea <ChevronRight :size="14" /></button><button v-else-if="normalizedCandidateStatus(selectedCandidate) === 'candidate'" class="button button--primary" @click="selectCandidate(selectedCandidate)">Turn into idea <ChevronRight :size="14" /></button></div>
      </section>
    </div>

    <div v-if="selectedEvidence" class="modal-backdrop" data-testid="evidence-details" @click.self="selectedEvidence = null">
      <section class="modal evidence-modal" role="dialog" aria-modal="true" :aria-labelledby="`evidence-title-${selectedEvidence.id}`">
        <div class="modal__header"><div><span class="eyebrow">Research evidence · {{ selectedEvidence.source_type || 'web' }}</span><h2 :id="`evidence-title-${selectedEvidence.id}`">{{ selectedEvidence.title }}</h2><p>Retrieved {{ runDate(selectedEvidence.retrieved_at) }} · relevance {{ Math.round((selectedEvidence.relevance || 0) * 100) }}% · confidence {{ Math.round((selectedEvidence.confidence || 0) * 100) }}%</p></div><button class="icon-button icon-button--plain" aria-label="Close evidence details" @click="selectedEvidence = null"><X :size="18" /></button></div>
        <div class="modal__body evidence-detail"><UiMarkdownText :text="selectedEvidence.content || selectedEvidence.excerpt || 'No excerpt was returned for this source.'" /><aside v-if="selectedEvidenceLinks.length"><h3>Source links</h3><a v-for="link in selectedEvidenceLinks" :key="link.url" :href="link.url" target="_blank" rel="noopener noreferrer">{{ link.label || link.url }} <ExternalLink :size="13" /></a></aside></div>
        <div class="modal__footer"><button class="button" @click="selectedEvidence = null">Close</button><a v-if="selectedEvidence.url" class="button button--primary" :href="selectedEvidence.url" target="_blank" rel="noopener noreferrer">Open original <ExternalLink :size="14" /></a></div>
      </section>
    </div>
  </div>
</template>
...
.candidate-card--idea_created{border-color:#d9c2df;background:#fcf8fd}.candidate-card--hidden{opacity:.72}.candidate-recommendation{display:flex;flex-wrap:wrap;gap:6px;margin-top:9px}.candidate-recommendation span{display:inline-flex;align-items:center;gap:4px;padding:4px 6px;border-radius:7px;background:white;color:var(--primary-700);font-size:8px;font-weight:700;text-transform:capitalize}.candidate-recommendation--detail{margin:8px 0 18px}.candidate-recommendation--detail span{background:var(--primary-50)}.research-history{margin-top:15px}.research-history__list{display:grid;gap:7px}.research-history__list>button{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:5px 16px;padding:12px;border:1px solid var(--border);border-radius:10px;background:var(--surface-soft);color:inherit;text-align:left;transition:.15s}.research-history__list>button:hover,.research-history__list>button.active{border-color:var(--primary-300);background:white}.research-history__list>button>div:first-child{display:grid;gap:4px}.research-history__list strong{font-size:10px}.research-history__list span,.research-history__list small{color:var(--muted);font-size:8px}.research-history__metrics{display:flex;align-items:center;justify-content:flex-end;gap:9px}.research-history__list small{grid-column:1/-1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}@media(max-width:760px){.research-history__list>button{grid-template-columns:1fr}.research-history__metrics{justify-content:flex-start;flex-wrap:wrap}.research-history__list small{grid-column:auto;white-space:normal}}
.candidate-type{padding:2px 5px;border-radius:99px;background:var(--primary-50);color:var(--primary-700)!important;font-weight:800;text-transform:capitalize}.candidate-summary{color:var(--muted);font-size:10px;line-height:1.5}.candidate-detail__main>.markdown-text{color:var(--muted-strong);font-size:9px;line-height:1.6}.evidence-row .markdown-text{display:-webkit-box;overflow:hidden;margin:3px 0;color:var(--muted);font-size:8px;line-height:1.45;-webkit-box-orient:vertical;-webkit-line-clamp:2}
</style>
