...
export default defineNuxtRouteMiddleware(async (to) => {
  ...
  if (isPublic) {
    if (auth.accessToken.value && ['/login', '/register'].includes(to.path)) return navigateTo('/')
    return
  }
  ...
  if (to.path.startsWith('/admin') && !auth.user.value?.is_platform_admin) return navigateTo('/')
})
