...
import { Check, CircleDollarSign, Clapperboard, ImageIcon, Pause, Pencil, Play, Save, Settings2, ShieldCheck, Upload, Volume2, X } from 'lucide-vue-next'

type SettingsTab = 'general' | 'brand' | 'automation' | 'video' | 'budget' | 'compliance'
type VideoType = 'ugc_creator' | 'storytelling' | 'cinematic' | 'motion_graphics'
const { api, apiBase, projectId } = useApi()
const { show } = useToast()
const route = useRoute()
const router = useRouter()
const validTabs: SettingsTab[] = ['general', 'brand', 'automation', 'video', 'budget', 'compliance']
...
const tabs: { key: SettingsTab, label: string }[] = [
  ...
  { key: 'video', label: 'Video defaults' },
...
const help = {
  ...
  automation: 'Off does nothing; Research only keeps the candidate backlog full; Create scripts selects top ideas and pauses before video; Create videos also renders them; Publish sends passed videos to every connected channel.',
...
const projectForm = reactive({ name: '', timezone: '', automation_mode: 'research_only', videos_per_week: 3, average_duration_seconds: 30, daily_cap: 1, weekly_cap: 3, minimum_gap_hours: 18, allowed_start: '09:00', allowed_end: '18:00', quiet_start: '', quiet_end: '', pause_all_publications: false, backlog_target: 150, research_recency_days: 30, research_schedule_enabled: true, research_interval_hours: 24, monthly_budget: 120, readiness_manual: 70, readiness_autopublish: 88, confidence_threshold: 0.65 })
const videoDefaults = reactive<Record<VideoType, { audio_mode: string, native_voice_preset: string, continue_scenes: boolean }>>({
  ugc_creator: { audio_mode: 'veo_native', native_voice_preset: 'warm_conversational', continue_scenes: true },
  storytelling: { audio_mode: 'veo_native', native_voice_preset: 'warm_conversational', continue_scenes: false },
  cinematic: { audio_mode: 'veo_native', native_voice_preset: 'warm_conversational', continue_scenes: false },
  motion_graphics: { audio_mode: 'veo_native', native_voice_preset: 'warm_conversational', continue_scenes: false },
})
const videoTypes: { key: VideoType, label: string }[] = [{ key: 'ugc_creator', label: 'Creator UGC' }, { key: 'storytelling', label: 'Storytelling sketch' }, { key: 'cinematic', label: 'Cinematic' }, { key: 'motion_graphics', label: 'Motion graphics' }]
...
function hydrate() {
  ...
  Object.assign(projectForm, {
    ...
    pause_all_publications: Boolean(project.settings?.publishing?.pause_all_publications), backlog_target: project.settings?.research?.backlog_target ?? 150,
  ...
  for (const type of videoTypes) Object.assign(videoDefaults[type.key], project.settings?.video_defaults?.[type.key] || {})
...
async function save() {
  saving.value = true
  try {
    await api(`/v1/projects/${projectId.value}`, { method: 'PATCH', body: { name: projectForm.name, timezone: projectForm.timezone, automation_mode: projectForm.automation_mode, settings: { research: { backlog_target: projectForm.backlog_target, recency_days: projectForm.research_recency_days, max_candidates: 50 }, production: { videos_per_week: projectForm.videos_per_week, average_duration_seconds: projectForm.average_duration_seconds }, video_defaults: videoDefaults, publishing: { daily_cap: projectForm.daily_cap, weekly_cap: projectForm.weekly_cap, minimum_gap_hours: projectForm.minimum_gap_hours, allowed_time_windows: [{ weekdays: [0, 1, 2, 3, 4, 5, 6], start: projectForm.allowed_start, end: projectForm.allowed_end }], blackout_periods: projectForm.quiet_start && projectForm.quiet_end ? [{ start: new Date(projectForm.quiet_start).toISOString(), end: new Date(projectForm.quiet_end).toISOString() }] : [], pause_all_publications: projectForm.pause_all_publications }, budget: { monthly_usd: projectForm.monthly_budget }, scoring: { readiness_manual: projectForm.readiness_manual, readiness_autopublish: projectForm.readiness_autopublish, confidence: projectForm.confidence_threshold } } } })
    const researchProfileBody = { name: 'Automatic idea discovery', objective: 'Find fresh, evidence-backed short-form topics for this project and its primary audience.', interval_hours: projectForm.research_interval_hours, timezone: projectForm.timezone, recency_days: projectForm.research_recency_days, max_candidates: 50 }
    const currentResearchProfile = data.value?.researchProfile
    const researchEnabled = projectForm.automation_mode !== 'off'
    if (currentResearchProfile) await api(`/v1/research-profiles/${currentResearchProfile.id}`, { method: 'PATCH', body: { ...researchProfileBody, status: researchEnabled ? 'active' : 'paused' } })
    else if (researchEnabled) await api(`/v1/projects/${projectId.value}/research-profiles`, { method: 'POST', body: researchProfileBody })
  ...
  <div v-if="data">
    ...
    <div class="settings-grid">
      ...
      <div class="settings-content">
        ...
        <UiAppCard v-else-if="activeTab === 'video'">
          <div class="section-heading"><div><h2>Video defaults</h2><p>Automation and new production forms use these settings per format.</p></div><Clapperboard :size="18" /></div>
          <div class="video-default-list"><div v-for="type in videoTypes" :key="type.key" class="video-default-row"><strong>{{ type.label }}</strong><template v-if="editing"><label>Voice<select v-model="videoDefaults[type.key].audio_mode"><option value="veo_native">Premium · Veo native</option><option value="google_tts">Standard · Google TTS</option></select></label><label>Voice profile<select v-model="videoDefaults[type.key].native_voice_preset" :disabled="videoDefaults[type.key].audio_mode !== 'veo_native'"><option value="warm_conversational">Warm conversational</option><option value="calm_expert">Calm expert</option><option value="bright_creator">Bright creator</option><option value="grounded_storyteller">Grounded storyteller</option></select></label><label class="checkbox-row"><input v-model="videoDefaults[type.key].continue_scenes" type="checkbox" /> Continue recurring roles</label></template><template v-else><span>{{ videoDefaults[type.key].audio_mode === 'veo_native' ? 'Premium · Veo native' : 'Standard · Google TTS' }}</span><span>{{ videoDefaults[type.key].continue_scenes ? 'Role continuation on' : 'Independent scenes' }}</span></template></div></div>
        </UiAppCard>
...
.video-default-list{display:grid;gap:9px}.video-default-row{display:grid;grid-template-columns:1.1fr 1fr 1fr 1fr;align-items:center;gap:12px;padding:13px;border:1px solid var(--border);border-radius:10px}.video-default-row strong{font-size:10px}.video-default-row>span{color:var(--muted);font-size:9px}.video-default-row label{display:grid;gap:4px;color:var(--muted);font-size:8px}.video-default-row select{padding:8px;border:1px solid var(--border);border-radius:8px;background:white;font-size:8px}@media(max-width:800px){.video-default-row{grid-template-columns:1fr}}
</style>
