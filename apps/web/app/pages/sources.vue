...
async function saveSource() {
  saving.value = true
  try {
    await api(`/v1/projects/${projectId.value}/sources`, { method: 'POST', body: { type: form.type, name: form.name, url: form.url || undefined, config: { generation_policy: 'research_then_approval', rights_confirmed: true, poll_interval_minutes: 60, include_url_patterns: ['*'] } } })
...
async function saveContent() {
  saving.value = true
  try {
    const body: any = { source_type: contentForm.source_type, title: contentForm.title, content_markdown: contentForm.content_markdown, rights_confirmed: true, processing: { run_research: contentForm.run_research } }
    if (contentForm.source_type === 'url') body.canonical_url = contentForm.canonical_url
    await api(`/v1/projects/${projectId.value}/source-items`, { method: 'POST', headers: { 'Idempotency-Key': `ui-source-${Date.now()}` }, body })
    show('Content accepted', 'The item was normalized, checked for duplicates and queued for the selected workflow.', 'success')
