...
test('creates an idea, renders a mock production, approves and publishes it', async ({ page }) => {
  ...
  const continuation = page.getByLabel(/Continue recurring roles from their own previous Veo scenes/)
  await expect(continuation).toBeChecked()
  const targetDuration = page.getByLabel('Target duration')
  await expect(targetDuration).not.toHaveAttribute('max')
  await targetDuration.fill('40')
  await page.getByLabel('Preferred scene count').fill('5-8')
  await expect(page.getByText(/duration requires about 6 generated fragments/)).toBeVisible()
  await expect(page.getByTestId('start-generation')).toBeEnabled()
  await targetDuration.fill('30')
  await page.getByLabel('Preferred scene count').fill('4-6')
  await page.locator('.video-type-grid > label').filter({ hasText: 'Storytelling / sketch' }).click()
  await expect(page.getByLabel('Storytelling / sketch', { exact: true })).toBeChecked()
  await expect(continuation).not.toBeChecked()
  await continuation.check()
  await expect(continuation).toBeChecked()
  await continuation.uncheck()
  await page.locator('.audio-mode-switch > label').filter({ hasText: 'Veo 3 native voice' }).click()
  await expect(page.getByLabel('Veo 3 native voice', { exact: true })).toBeChecked()
  ...
  expect(generationPayload.generation_start_mode).toBe('immediate')
