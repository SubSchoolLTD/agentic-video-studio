...
import { AlertTriangle, ArrowLeft, Check, CheckCircle2, CircleDollarSign, Clock3, Download, ExternalLink, FileText, Film, ImageIcon, Lock, Maximize2, Play, Pencil, RadioTower, RotateCcw, Save, ShieldCheck, Sparkles, UsersRound, WandSparkles, X } from 'lucide-vue-next'
...
const selectedTakes = reactive<Record<string, string>>({})
const latestTakeIds = new Map<string, string>()
...
const promptScene = ref<any>(null)
const promptMode = ref<'edit' | 'regenerate'>('edit')
const productionBusy = computed(() => !terminal.value || Boolean(job.value?.active_regeneration_id || job.value?.active_reassembly_id))
let ticking = false
const mediaUrls = new Map<string, { url: string, savedAt: number }>()
...
const previewVersion = computed(() => video.value?.versions?.find((item: any) => item.id === selectedVersionId.value) || video.value?.versions?.find((item: any) => item.id === video.value?.latest_version_id) || [...(video.value?.versions || [])].sort((a: any, b: any) => Number(b.version) - Number(a.version))[0])
...
const subtitleAssets = computed(() => previewVersion.value?.subtitle_assets || [])
const latestFormatVersion = computed(() => [...(video.value?.versions || [])].filter((item:any) => item.aspect_ratio === previewVersion.value?.aspect_ratio).sort((a:any,b:any) => b.version-a.version)[0])
const selectionChanged = computed(() => scenes.value.some((scene:any) => sceneAttempt(scene)?.id !== latestFormatVersion.value?.scene_attempt_ids?.[scene.id]))
const canAssemble = computed(() => !productionBusy.value && scenes.value.length > 0 && scenes.value.every((scene:any) => Boolean(sceneAttempt(scene)?.id)))
...
async function loadVideo() {
  if (job.value?.video_id) {
    const next = await api<any>(`/v1/videos/${job.value.video_id}`)
    // Polling must not restart a playing immutable render by rotating its signed URL.
    for (const version of next.versions || []) {
      const cached = mediaUrls.get(version.id)
      if (cached && Date.now() - cached.savedAt < 10 * 60 * 1000) version.render_url = cached.url
      else if (version.render_url) mediaUrls.set(version.id, { url: version.render_url, savedAt: Date.now() })
    }
    for (const scene of next.scenes || []) {
      for (const take of scene.attempt_history || scene.attempts || []) {
        const cached = mediaUrls.get(take.id)
        if (cached && Date.now() - cached.savedAt < 10 * 60 * 1000) take.preview_url = cached.url
        else if (take.preview_url) mediaUrls.set(take.id, { url: take.preview_url, savedAt: Date.now() })
      }
      for (const ratio of job.value.aspect_ratios || ['9:16']) {
        const latest = (scene.attempt_history || scene.attempts || []).find((take:any) => take.aspect_ratio === ratio)
        const key = `${scene.id}:${ratio}`
        if (latest && latestTakeIds.get(key) !== latest.id) {
          latestTakeIds.set(key, latest.id)
          selectedTakes[key] = latest.id
        }
      }
    }
    const latestChanged = next.latest_version_id !== video.value?.latest_version_id
    video.value = next
    if (latestChanged || !next.versions?.some((item: any) => item.id === selectedVersionId.value))
      selectedVersionId.value = next.latest_version_id || previewVersion.value?.id || ''
  }
}
...
async function tick() {
  ...
  ticking = true
  try {
    const revision = job.value?.updated_at
    await refreshJob()
    if (job.value?.video_id && (!video.value || revision !== job.value.updated_at)) await loadVideo()
  } finally { ticking = false }
}
function startPolling() {
  if (!poller) poller = setInterval(() => void tick().catch(() => {}), 3000)
}
onMounted(() => {
...
function regenerate(scene: any) {
  previewScene.value = null
  promptMode.value = 'regenerate'
  promptScene.value = scene
}
function editPrompt(scene: any) {
  previewScene.value = null
...
function takeKey(scene: any) { return `${scene.id}:${previewVersion.value?.aspect_ratio || '9:16'}` }
function sceneTakes(scene: any) { return (scene.attempt_history || scene.attempts || []).filter((item:any) => item.aspect_ratio === (previewVersion.value?.aspect_ratio || '9:16')) }
function sceneAttempt(scene: any) { const takes = sceneTakes(scene); return takes.find((item:any) => item.id === selectedTakes[takeKey(scene)]) || takes[0] }
function useRenderTakes() {
  for (const scene of scenes.value) {
    const id = previewVersion.value?.scene_attempt_ids?.[scene.id]
    if (sceneTakes(scene).some((take:any) => take.id === id)) selectedTakes[takeKey(scene)] = id
  }
  activeTab.value = 'storyboard'
}
function resetTakes() { for (const scene of scenes.value) selectedTakes[takeKey(scene)] = sceneTakes(scene)[0]?.id || '' }
async function rebuildVideo() {
  if (!canAssemble.value) return
  assembling.value = true
  try {
    await api(`/v1/videos/${video.value.id}/reassemble`, { method: 'POST', body: {
      base_version_id: video.value.latest_version_id, aspect_ratio: previewVersion.value.aspect_ratio,
      selections: scenes.value.map((scene:any) => ({ scene_id: scene.id, attempt_id: sceneAttempt(scene).id })),
    } })
    await refreshJob(); startPolling()
    show('Rebuilding final video', 'Your chosen takes will become a new render. No Veo scene generation is started.', 'success')
  } catch (error:any) { show('Could not rebuild video', error.message, 'error') }
...
function openSceneEditor(scene: any) { if (job.value?.status !== 'awaiting_script_review') return editPrompt(scene); editingScene.value = scene; Object.assign(sceneDraft, JSON.parse(JSON.stringify(scene))) }
...
<template>
  <div v-if="job">
    ...
    <div v-if="video?.versions?.length" class="render-history-actions"><span>Final render history: v{{ previewVersion?.version }} · {{ previewVersion?.aspect_ratio }}</span><button class="button button--small" :disabled="productionBusy" @click="useRenderTakes">Use this render's scenes</button></div>
    <div v-if="video && (selectionChanged || job.last_reassembly_error)" class="assembly-bar"><div><strong>{{ job.last_reassembly_error ? 'Rebuild interrupted' : 'Your scene selection differs from the latest final render' }}</strong><p>{{ job.last_reassembly_error || 'Preview any take, then rebuild when you are happy. Existing clips are reused; no Veo generation. Previous final versions are kept.' }}</p></div><button class="button" :disabled="productionBusy" @click="resetTakes">Latest takes</button><button class="button button--primary" :disabled="!canAssemble || assembling" @click="rebuildVideo">{{ assembling || job.active_reassembly_id ? 'Rebuilding…' : 'Rebuild final video' }}</button></div>
    <div v-if="job.last_regeneration_error" class="regeneration-notice" role="alert">Scene regeneration failed: {{ job.last_regeneration_error }}. The previous video is preserved. Open Storyboard to request another take and review its cost.</div>
    <div v-else-if="productionBusy && job.video_id" class="regeneration-notice">Updating this production. The preview remains the previous completed version until the new render is ready.</div>
    ...
    <div v-else-if="activeTab === 'storyboard'" class="scene-grid workspace-content"><UiAppCard v-for="scene in scenes" :key="scene.id" class="scene-card"><div class="scene-card__preview"><video v-if="scenePreviewUrl(scene)" :key="sceneAttempt(scene)?.id" :src="scenePreviewUrl(scene)" controls playsinline preload="metadata" :data-testid="`scene-preview-${scene.position}`" /><div v-else class="scene-card__missing"><Film :size="20" /><span>Scene media unavailable</span></div><span class="scene-number">{{ scene.position || scene.id.split('_').at(-1) }}</span><button v-if="scenePreviewUrl(scene)" class="scene-expand" aria-label="Open scene preview" @click="previewScene = scene"><Maximize2 :size="13" /></button></div><div class="scene-card__head"><div><strong>Scene {{ scene.position || '—' }}</strong><span>{{ scene.duration_target }} sec · take {{ sceneAttempt(scene)?.attempt || scene.attempt }}</span></div><UiStatusBadge :status="scene.status" /></div><label class="take-selector">Scene version<select v-model="selectedTakes[takeKey(scene)]" :aria-label="'Scene ' + scene.position + ' version'" :disabled="productionBusy"><option v-for="(take, index) in sceneTakes(scene)" :key="take.id" :value="take.id">Take {{ take.attempt }}{{ index === 0 ? ' · Latest' : '' }}</option></select></label><p>{{ sceneAttempt(scene)?.narration ?? scene.narration }}</p><small v-if="scene.prompt_edit_pending">Prompt updated · this preview is the previous take</small><div class="scene-speech" :class="{ 'scene-speech--failed': speechQa(scene).passed === false || voiceQa(scene).passed === false }"><ShieldCheck :size="13" /><span v-if="job.audio_mode === 'veo_native'">Speech {{ speechQa(scene).passed ? `${Math.round((speechQa(scene).coverage || 0) * 100)}%` : 'failed' }} · voice {{ voiceQa(scene).mode === 'reference_voice' ? 'reference' : voiceQa(scene).passed ? 'AI check passed' : 'changed' }}</span><span v-else>Timing preflight: {{ speechQa(scene).passed === false ? 'too long' : 'fits the scene' }}</span></div><details v-if="speechQa(scene).transcript"><summary>Actual transcript</summary><small>{{ speechQa(scene).transcript }}</small></details><details><summary>Visual prompt</summary><small>{{ sceneAttempt(scene)?.visual_prompt || scene.visual_prompt }}</small></details><button class="button button--small scene-edit-button" :disabled="productionBusy" @click="editPrompt(scene)"><Pencil :size="13" /> Edit scene</button><button class="button button--small" :disabled="productionBusy || !job.video_id" @click="regenerate(scene)"><RotateCcw :size="13" /> {{ regenerateId === scene.id ? 'Queued…' : 'Regenerate scene' }}</button></UiAppCard></div>
    ...
    <div v-if="previewScene" class="modal-backdrop" @click.self="previewScene = null"><section class="modal scene-modal"><div class="modal__header"><div><h2>Scene {{ previewScene.position }}</h2><p>{{ previewScene.duration_target }} seconds · take {{ sceneAttempt(previewScene)?.attempt || previewScene.attempt }} · {{ previewVersion?.aspect_ratio || '9:16' }}</p></div><button class="icon-button icon-button--plain" aria-label="Close scene preview" @click="previewScene = null"><X :size="18" /></button></div><div class="modal__body"><video :src="scenePreviewUrl(previewScene)" controls autoplay playsinline data-testid="scene-modal-video" /><p>{{ sceneAttempt(previewScene)?.narration ?? previewScene.narration }}</p><div class="scene-speech"><ShieldCheck :size="14" /><span>{{ speechQa(previewScene).passed ? 'Speech and timing QA passed' : speechQa(previewScene).issues?.join(' · ') || 'Timing preflight passed' }}</span></div></div><div class="modal__footer"><button class="button" @click="previewScene = null">Close</button><button class="button button--primary" :disabled="productionBusy || !job.video_id" @click="regenerate(previewScene); previewScene = null"><RotateCcw :size="13" /> Regenerate this scene</button></div></section></div>
    ...
    <ProductionSceneEditor v-if="promptScene" :key="promptScene.id" :scene="promptScene" :job="job" :mode="promptMode" @close="promptScene = null" @saved="sceneChanged" @queued="sceneChanged" />
  </div>
</template>

<style scoped>
.take-selector{display:grid;gap:6px;margin:12px 0;font-size:12px;color:var(--muted-strong)}.take-selector select{width:100%}.assembly-bar{display:flex;align-items:center;gap:14px;flex-wrap:wrap;background:var(--primary-50);border:1px solid var(--primary-200);border-radius:14px;padding:18px;margin-top:18px}.assembly-bar>div{flex:1;min-width:220px}.assembly-bar p{margin:6px 0 0;font-size:12px;line-height:1.6}.render-history-actions{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-top:12px;font-size:12px;color:var(--muted-strong)}
...
.scene-edit-button{margin-bottom:8px}.regeneration-notice{margin-top:14px;padding:12px;border-radius:10px;background:var(--primary-50);color:var(--primary-700);font-size:12px;line-height:1.5}
</style>
