...
import { ArrowUpRight, Check, ChevronRight, CircleHelp, Clock3, ExternalLink, RadioTower, Search, ShieldCheck, Sparkles, X } from 'lucide-vue-next'
      ...
      <UiAppCard>
        ...
        <div v-else class="candidate-list">
          <article v-for="candidate in candidates" :key="candidate.id" class="candidate-card">
            ...
            <div class="candidate-card__body"><div class="candidate-card__meta"><UiStatusBadge :status="candidate.status" /><span><Clock3 :size="12" /> fresh 7 days</span><span><ShieldCheck :size="12" /> {{ Math.round((candidate.score_confidence || 0) * 100) }}% confidence</span></div><h3>{{ candidate.title }}</h3><p>{{ candidate.angle }}</p><div class="candidate-card__footer"><span>{{ candidate.source_ids?.length || 0 }} cited sources</span><button class="text-link" @click="selectCandidate(candidate.id)">Turn into idea <ChevronRight :size="13" /></button></div></div>
          </article>
        </div>
      </UiAppCard>
