  ...
  <div>
    ...
    <div v-if="selectedCandidate" class="modal-backdrop" data-testid="candidate-details" @click.self="selectedCandidate = null">
      <section class="modal candidate-modal">
        ...
        <div class="modal__body candidate-detail"><div class="candidate-detail__main"><h3>Editorial angle</h3><p>{{ selectedCandidate.angle }}</p><h3>Recommended production</h3><p>{{ selectedCandidate.format_rationale || 'The recommendation balances the message, audience and available evidence.' }}</p><div class="candidate-recommendation candidate-recommendation--detail"><span><Film :size="13" /> {{ String(selectedCandidate.recommended_visual_mode || 'ugc_creator').replaceAll('_', ' ') }}</span><span>{{ selectedCandidate.recommended_duration_seconds || 30 }} seconds</span><span>{{ selectedCandidate.recommended_scene_count_min || 4 }}–{{ selectedCandidate.recommended_scene_count_max || 6 }} scenes</span></div><h3>Supported claims</h3><ul v-if="selectedCandidate.supported_claims?.length"><li v-for="claim in selectedCandidate.supported_claims" :key="claim.id || claim.claim">{{ claim.claim || claim.text }}</li></ul><p v-else class="muted-copy">No factual claim is promoted without attached evidence.</p><h3>Unresolved questions</h3><ul v-if="selectedCandidate.unresolved_questions?.length"><li v-for="question in selectedCandidate.unresolved_questions" :key="question">{{ question }}</li></ul><p v-else class="muted-copy">No unresolved questions were reported.</p></div><aside><dl><div><dt>Status</dt><dd>{{ normalizedCandidateStatus(selectedCandidate).replaceAll('_', ' ') }}</dd></div><div><dt>Audience</dt><dd>{{ selectedCandidate.audience }}</dd></div><div><dt>Objective</dt><dd>{{ selectedCandidate.objective }}</dd></div><div><dt>Editorial format</dt><dd>{{ selectedCandidate.format }}</dd></div><div><dt>Opportunity</dt><dd>{{ selectedCandidate.topic_opportunity_score }}</dd></div><div><dt>Confidence</dt><dd>{{ Math.round((selectedCandidate.score_confidence || 0) * 100) }}%</dd></div></dl><h3>Sources</h3><a v-for="source in selectedSources" :key="source.id" :href="source.url" target="_blank" rel="noreferrer">{{ source.title }} <ExternalLink :size="12" /></a></aside></div>
