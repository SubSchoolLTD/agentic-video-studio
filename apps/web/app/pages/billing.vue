...
async function startTopup() {
  ...
  try {
    const result = await api<any>('/v1/billing/topups/paypal', {
      method: 'POST',
      body: { amount_usd: Number(amountUsd.value).toFixed(2), promo_code: promoCode.value.trim() || null },
    })
...
<template>
  <div>
    ...
    <div class="metric-grid">
      <UiAppCard class="metric-card"><div class="metric-card__head"><span class="metric-card__icon metric-card__icon--purple"><WalletCards :size="17" /></span><span>Available balance</span></div><strong>{{ money(summary?.balance_usd) }}</strong><small>USD · no subscription</small></UiAppCard>
      <UiAppCard class="metric-card"><div class="metric-card__head"><span class="metric-card__icon metric-card__icon--green"><BadgeDollarSign :size="17" /></span><span>Payment model</span></div><strong>Usage based</strong><small>Provider cost + 20%</small></UiAppCard>
    </div>

    <div class="billing-grid">
      <UiAppCard class="topup-card">
        <div class="section-heading"><div><h2>Top up with PayPal</h2><p>Minimum $12. A promo code can add a balance bonus.</p></div><CircleDollarSign :size="19" /></div>
        <form class="topup-form" @submit.prevent="startTopup">
          ...
          <label><span>Promo code <small>optional</small></span><div class="promo-input"><Gift :size="16" /><input v-model="promoCode" maxlength="64" placeholder="FRAME-2026" aria-label="Promo code"></div></label>
