...
test('research keeps action candidates focused and exposes created and hidden history', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: /Research/ }).locator('.nav-link__badge')).toHaveText(String(countBefore))
  await expect(cards.first().locator('.candidate-type')).toBeVisible()
  await expect(cards.first().locator('.candidate-summary strong')).toBeVisible()
  ...
  await expect(details.getByRole('heading', { name: 'Audience insight' })).toBeVisible()
  await expect(details.getByRole('heading', { name: 'Problem and proposed solution' })).toBeVisible()
  await expect(details.getByRole('heading', { name: 'Value and viral mechanism' })).toBeVisible()
  ...
  await expect(page.getByRole('link', { name: /Research/ }).locator('.nav-link__badge')).toHaveText(String(countBefore - 1))
