...
test('uploads a reusable character and starts native-audio UGC from an idea', async ({ page }) => {
  ...
  await page.getByLabel('Default visual style').selectOption('ugc_creator')
  await page.getByLabel('Veo 3 native voice').check({ force: true })
  await page.getByLabel('Native voice profile').selectOption('grounded_storyteller')
  ...
  await expect(card.getByText('ugc creator')).toBeVisible()
  await expect(card.getByText('Veo native voice')).toBeVisible()
  ...
  await expect(page.getByText(/ugc creator · Veo native speech/)).toBeVisible({ timeout: 30_000 })
  await page.getByRole('button', { name: 'storyboard', exact: true }).click()
  await expect(page.getByText(/Speech 100% · voice reference/).first()).toBeVisible()
})
