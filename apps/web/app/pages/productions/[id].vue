...
onMounted(() => {
  ...
  if (!terminal.value) poller = setInterval(() => void tick(), 600)
})
      ...
      <UiAppCard class="timeline-card">
        ...
        <ol class="stage-list">
          <li v-for="(stage, index) in job.stages" :key="stage.name" :class="[`stage-list__item--${stage.status}`]">
            ...
            <UiStatusBadge :status="stage.status" />
          </li>
        </ol>
      </UiAppCard>
    </div>
