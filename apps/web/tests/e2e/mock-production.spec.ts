...
test('creates an idea, renders a mock production, approves and publishes it', async ({ page }) => {
  ...
  const continuation = page.getByLabel(/Continue each scene from the previous Veo video/)
  await expect(continuation).toBeChecked()
  await page.getByLabel('Storytelling / sketch', { exact: true }).check({ force: true })
  await expect(continuation).not.toBeChecked()
  await continuation.check()
  await expect(continuation).toBeChecked()
  await continuation.uncheck()
  await page.getByLabel('Veo 3 native voice', { exact: true }).check({ force: true })
  await page.getByLabel('Native voice profile').selectOption('calm_expert')
  ...
  expect(generationPayload.visual_mode).toBe('storytelling')
  expect(generationPayload.audio_mode).toBe('veo_native')
  expect(generationPayload.continue_scenes).toBe(false)
  expect(generationPayload.native_voice_preset).toBe('calm_expert')
