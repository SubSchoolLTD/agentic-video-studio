...
import { Activity, ArrowRight, CalendarClock, CircleDollarSign, Clapperboard, FlaskConical, Pause, Play, Plus, RadioTower, Sparkles, WandSparkles } from 'lucide-vue-next'
...
const { data, refresh, status } = await useAsyncData('overview-data', async () => {
  ...
  default: () => ({
    project: { name: 'SubSchool', status: 'active', autopilot_paused: false, settings: { budget: { used_usd: 24.8, monthly_usd: 120 } } },
    analytics: { kpis: { published: 0, videos_ready: 0, awaiting_approval: 0, active_jobs: 0, idea_backlog: 7, budget_used_usd: 24.8, budget_limit_usd: 120 }, patterns: [] },
...
async function quickGenerate() {
  ...
  try {
    const result = await api<any>(`/v1/projects/${projectId.value}/generation-jobs`, {
      ...
      body: {
        title: 'Turn one lesson into three reusable learning assets',
...
<template>
  <div>
    <UiPageHeader eyebrow="SubSchool workspace" title="Good morning, Maksim" description="Your evidence-first production desk is healthy. Three ideas are ready for review and the content backlog is on target.">
    ...
    <div class="metric-grid">
      ...
      <UiAppCard class="metric-card"><span class="metric-card__icon"><CircleDollarSign :size="18" /></span><span class="metric-card__label">Monthly usage</span><div class="metric-card__value"><strong>${{ Number(kpis.budget_used_usd || 0).toFixed(0) }}</strong><span>of ${{ kpis.budget_limit_usd || 0 }}</span></div><UiProgressBar :value="budgetPercent" style="margin-top: 9px" /></UiAppCard>
    </div>
    ...
    <div class="overview-layout">
      <div class="stack">
        <UiAppCard>
          ...
          <div v-else-if="!recentJobs.length" class="compact-empty"><Clapperboard :size="24" /><div><strong>No productions yet</strong><span>Start with the prepared SubSchool idea.</span></div><button class="button button--small" @click="quickGenerate"><Plus :size="14" /> Generate</button></div>
            ...
            <table class="data-table">
              ...
              <tbody>
                <tr v-for="job in recentJobs" :key="job.id">
                  <td><div class="table-title"><strong>{{ job.title || 'SubSchool production' }}</strong><span>{{ job.aspect_ratios?.join(' · ') }} · {{ job.target_duration_seconds }} sec</span></div></td>
      ...
      <div class="stack">
        <UiAppCard class="next-action-card">
          ...
          <span class="eyebrow">Next scheduled run</span><h2>Teacher creator signals</h2><p>Parallel searches audience demand, fresh developments, primary evidence and competing saturation.</p>
          <div class="next-action-card__meta"><CalendarClock :size="14" /><span>Monday · 09:00 ET</span></div>
        ...
        <UiAppCard>
          ...
          <div class="learning-signal"><strong>Question hooks</strong><span class="learning-signal__delta">+12.4 percentile</span><p>Promising on short teacher explainers, but the sample is still too small for automatic strategy changes.</p><div class="confidence-row"><span>Confidence 58%</span><UiProgressBar :value="0.58" /></div><small>n = 7 · Demo data</small></div>
        </UiAppCard>
