...
def test_production_deploy_wires_youtube_without_per_user_secret_storage() -> None:
    ...
    assert "SOCIAL_BROWSER_SESSION_SECRET=social-browser-sessions" not in workflow
    assert "social-browser-sessions" not in workflow
    assert "TIKTOK_REDIRECT_URI,SOCIAL_BROWSER_SESSION_SECRET" in workflow
